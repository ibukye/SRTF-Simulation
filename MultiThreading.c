...
void gantt_chart(GanttLog *Log, int log_index) {
    ...
    for (int i = 1; i < log_index; i++) {
        if (Log[i].status == COMPLETED) continue; //if COMPLETED, just skip
...
int main() {
    ...
    for (int i = 0; i < num_of_process; i++) {
        ...
        if (pthread_create(&All_Process[i].thread, NULL, process_thread_function, &All_Process[i]) != 0) {
    ...
    while (completed_count < num_of_process) {
        ...
        if (shortest_index != -1) {
            ...
            Log[log_index++] = (GanttLog) { current_time, All_Process[shortest_index].ID, All_Process[shortest_index].RemainingBurstTime, RUNNING };
            ...
            // Print RUNNING log
            {
                ...
                sprintf(rem_str, "%d", All_Process[shortest_index].RemainingBurstTime);
                int w_pid = 12, w_stat = 14, w_rem = 16;
                int l_pid = (w_pid - (int)strlen(pid_str)) / 2; int r_pid = w_pid - (int)strlen(pid_str) - l_pid;
                int l_stat = (w_stat - (int)strlen(stat_str)) / 2; int r_stat = w_stat - (int)strlen(stat_str) - l_stat;
                int l_rem = (w_rem - (int)strlen(rem_str)) / 2; int r_rem = w_rem - (int)strlen(rem_str) - l_rem;

                printf("%5d |%*s%s%*s|%*s%s%*s|%*s%s%*s|\n", current_time, l_pid, "", pid_str, r_pid, "", l_stat, "", stat_str, r_stat, "", l_rem, "", rem_str, r_rem, "");
            }
            ...
            if (All_Process[shortest_index].RemainingBurstTime == 0) {
...

                {
                    ...
                    printf("%5d |%*s%s%*s|%*s%s%*s|%*s%s%*s|\n", current_time + 1, l_pid, "", pid_str, r_pid, "", l_stat, "", stat_str, r_stat, "", l_rem, "", rem_str, r_rem, "");
                }
            }
