...
void initialize_processes(int num_processes, Process *processes,
  ...
  for (int i = 0; i < num_processes; i++) {
    ...
    // Arrival Time
    snprintf(prompt, sizeof(prompt), "Arrival (0-%d): ", INT_MAX);
    int AT = read_int(prompt, 0, INT_MAX);

    // Burst Time
    snprintf(prompt, sizeof(prompt), "Burst (1-%d): ", INT_MAX);
...
void init_log_buffer(SimulationContext *ctx, int initial_capacity) {
  ctx->log_buffer = malloc(sizeof(GanttLog) * initial_capacity);
  if (ctx->log_buffer == NULL) {
    fprintf(stderr, "Error: Failed to allocate log buffer.\n");
    exit(EXIT_FAILURE);
  }
  ctx->log_capacity = initial_capacity;
  ctx->log_index = 0;
}

// Grow log buffer when necessary
void ensure_log_capacity(SimulationContext *ctx) {
  if (ctx->log_index < ctx->log_capacity) {
    return;
  }

  int new_capacity = ctx->log_capacity * 2;
  if (new_capacity <= ctx->log_capacity) {
    fprintf(stderr, "Error: Log buffer capacity overflow.\n");
    exit(EXIT_FAILURE);
  }

  GanttLog *new_buffer =
      realloc(ctx->log_buffer, sizeof(GanttLog) * new_capacity);
  if (new_buffer == NULL) {
    fprintf(stderr, "Error: Failed to grow log buffer.\n");
    exit(EXIT_FAILURE);
  }

  ctx->log_buffer = new_buffer;
  ctx->log_capacity = new_capacity;
}

void free_log_buffer(SimulationContext *ctx) {
  free(ctx->log_buffer);
  ctx->log_buffer = NULL;
  ctx->log_capacity = 0;
  ctx->log_index = 0;
...
void add_log(SimulationContext *ctx, int time, int pid, int remaining,
             process_state_t status) {
  ensure_log_capacity(ctx);
  ctx->log_buffer[ctx->log_index++] = (GanttLog){time, pid, remaining, status};
}
...
void run_scheduler(int num_processes, Process *processes,
  ...
  while (completed_count < num_processes) {
    ...
    if (shortest_index != -1) {
      if (processing_process_index != -1 &&
        ...
        if (processes[processing_process_index].status != STATE_COMPLETED) {
      ...
      if (processes[shortest_index].ResponseTime == -1) {
        ...
        snprintf(pid_str, sizeof(pid_str), "P%d", processes[shortest_index].ID);
      ...
      add_log(ctx, current_time, processes[shortest_index].ID,
              processes[shortest_index].RemainingBurstTime, STATE_RUNNING);

      {
        ...
        print_row(current_time, pid_str,
                  status_to_string(processes[shortest_index].status), rem_str);
      }

      ctx->tick_completed = 0;
      processes[shortest_index].run_permission = 1;
      check_pthread(pthread_cond_signal(&processes[shortest_index].my_cond),
                    "Signal process");
  ...
  for (int i = 0; i < num_processes; i++) {
    check_pthread(pthread_cond_signal(&processes[i].my_cond), "Signal finish");
  }
  check_pthread(pthread_mutex_unlock(&ctx->global_mutex),
...
cleanup:
  ...
  for (int i = 0; i < created_threads; i++) {
    check_pthread(pthread_cond_signal(&processes[i].my_cond), "Signal cleanup");
  }
  check_pthread(pthread_mutex_unlock(&ctx->global_mutex),
...
int main() {
  ...
  init_log_buffer(&ctx, INITIAL_LOG_CAPACITY);

  int num_processes = read_int("Enter number of processes (1-10): ", 1, 10);
